    ...
    def __init__(self, *ar, **kw):
        ...
        if self.key:
            ...
            if len(ar) and not self.name_pattern_list and not (self.path_list is None):
                ...
                # a pass search list is specified.
                assert self.name_pattern_list != None, 'self.name_pattern_list != None'
                log.debug(self.name_pattern_list)
            ...
            if len(founds) == 0: # 見つからない
                if len(ar):
                    name = self.name_src
                else:
                    name = self.name_pattern_list
                if self.required == True: # 必須.
    ...
    def _commit(self, idx, kconfig):
        ...
        try:
            ...
            if not isinstance(self.name, str):
                raise RuntimeError(
                    "expected str type for a file name value self.name={}".format(repr(self.name)))
            try:
